...
class ScheduledPageDr extends StatefulWidget {
  final String initialAddress;

  ScheduledPageDr({Key? key, required this.initialAddress}) : super(key: key);

  @override
  _ScheduledPageDrState createState() => _ScheduledPageDrState();
}

class _ScheduledPageDrState extends State<ScheduledPageDr> {
  ...
  try {
    ...
    for (String day in selectedDays) {
      ...
      Map<String, dynamic> requestData = {
        ...
        'capacity': passengers,
        'day': day,
        'date': date != null ? DateFormat('yyyy-MM-dd').format(date) : '0000-00-00',
        'hours': hour,
        'minutes': minute,
        'user_id': userId,
      };
      ...
      final response = await http.post(
        Uri.parse('http://192.168.68.113:5000/save_driver_route_scheduled'),
      ...
      if (response.statusCode == 200) {
        print('Driver route saved successfully for $day');
      } else {
        print('Failed to save driver route for $day. Status code: ${response.statusCode}');
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Your routes have been saved! Get ready to find your matches!'),
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            Icon(Icons.person),
                            Expanded(
                                child: TextField(controller: _numberController,
                                    decoration: InputDecoration(
...
void _navigateToPinsPage(BuildContext context) async {
  ...
  if (addressComponents.length >= 3) {
    ...
    print('Locality: $locality');
    print('Country: $country');
    print('id ${Driver.loggedInUserId}');
...
class InitialAddressPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return ScheduledPageDr(initialAddress: initialAddress);
  }
}
