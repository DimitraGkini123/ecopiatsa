...
import 'NowPinsDr.dart';
import 'driver.dart';

class MyPage extends StatefulWidget {
  final String initialAddress;

  MyPage({Key? key, required this.initialAddress}) : super(key: key);
...
class _MyPageState extends State<MyPage> {
  ...
  try {
    ...
    /*final response = await http.post(
      Uri.parse('http://147.102.200.23:5000/save_driver_route_now'),
      ...
      final response = await http.post(
      Uri.parse('http://192.168.68.113:5000/save_driver_route_now'),  //spiti
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(requestData),
...
print('Country: $country');
  ...
  child: ElevatedButton(
    onPressed: () async{
                    ...
                    _selectedDate , _hourController.text, _minuteController.text,
...
class InitialAddressPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return MyPage(initialAddress: initialAddress);
  }
}
